    ...
    newSession.innerHTML = `
        ...
        <div class="input-row">
            ...
            <div class="campus-location-field" style="display:none;">
                ...
                <input type="text" id="location-${sessionCounter}" class="location" list="campus-list" autocomplete="off" placeholder="Chọn địa điểm">
            </div>
...
document.getElementById('course-form').addEventListener('submit', function (e) {
    ...
    sessionElements.forEach(sessionElement => {
        ...
        if (!dayOfWeek || !time || !locationType) {
            isConflict = true;
            showAlert('Vui lòng điền đầy đủ thông tin cho tất cả các buổi học.', 'error');
            return;
        }
                ...
                for (const existingSession of existingCourse.sessions) {
                    if (newSession.dayOfWeek === existingSession.dayOfWeek) {
                        ...
                        if (Math.max(newStartTimeInMin, existingStartTimeInMin) < Math.min(newEndTimeInMin, existingEndTimeInMin)) {
                            isConflict = true;
                            showAlert(
                                `Lỗi: Lịch học bị trùng. Môn bạn đang cố gắng đăng ký trùng với môn **${existingCourse.courseName}** vào lúc **${existingSession.time}** vào ${existingSession.dayOfWeek}.`,
                                'error'
                            );
                            break;
                        }
        ...
        <div class="class-session-row">
            ...
            <div class="input-row">
                ...
                <div class="campus-location-field" style="display:none;">
                    ...
                    <input type="text" id="location-0" class="location" list="campus-list" autocomplete="off" placeholder="Chọn địa điểm">
...
function clearAllCourses() {
    ...
    renderTimetable();
    updateRegisteredCoursesList();
    showAlert('Đã xóa tất cả môn học.', 'success');
}
